...
// ValidateEVMChannelParams does validation of a newly created EVM channel. A EVM
// channel must be UNORDERED, use the correct port (by default 'evm'), and use the current
// supported version. Only 2^32 channels are allowed to be created.
func ValidateEVMChannelParams(
	ctx sdk.Context,
	keeper keeper.Keeper,
	...
	if channelSequence > uint64(math.MaxUint32) {
		return sdkerrors.Wrapf(types.ErrMaxEVMChannels, "channel sequence %d is greater than max allowed EVM channels %d", channelSequence, uint64(math.MaxUint32))
	}
...
func (im IBCModule) OnChanOpenInit(
	...
	if err := ValidateEVMChannelParams(ctx, im.keeper, order, portID, channelID, version); err != nil {
		return err
	}
...
func (im IBCModule) OnAcknowledgementPacket(
	...
	if err := im.keeper.OnAcknowledgementPacket(ctx, packet, tx, ack); err != nil {
		return err
	}
